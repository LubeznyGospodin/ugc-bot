...
_UNISEX_FIRST = {"саша", "женя", "валя", "слава"}
...
def guess_gender(full_name: str | None) -> str | None:
    ...
    if len(words) >= 2:  # есть фамилия — она решает
        surn = words[1]
        if surn in _MALE_SURNAMES:
            # Такая фамилия не склоняется и бывает у обоих: «Мария Кучма» — женщина.
            # Поэтому решает имя, а не фамилия.
            return _by_first_name(words[0])
        if surn.endswith(("ова", "ева", "ёва", "ина", "ына", "ская", "цкая", "ая", "яя")):
            return "female"
        if surn.endswith(("ов", "ев", "ёв", "ин", "ын", "ский", "цкий", "ой", "ый", "ий")):
            return "male"
        if surn.endswith(("а", "я")):
            return "female"
        return "male"

    return _by_first_name(words[0])  # одно слово — судим по имени
