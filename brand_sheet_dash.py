...
put(2, 7, "● обновляется каждое утро")
...
stages = [
    ...
    (12, "В КОРЗИНУ", "=SUM('WB · ROI'!E2:E)", "из перехода", '=IFERROR(D12/D11;"")', funnel_wait(61, 38, "$D$12")),
    (13, "ЗАКАЗЫ", "=SUM('WB · ROI'!F2:F)", "из корзины", '=IFERROR(D13/D12;"")', funnel_wait(48, 38, "$D$13")),
...
WBP = "'WB запросы'!$I$2:$I"    # период данных WB API (пусто = данных ещё нет)
put(15, 2, f'=IF(COUNTA({WBP})=0;'
           f'"☁️ Переходы · Корзина · Заказы · Выручка появятся, как только пойдёт трафик по #WW";'
           f'"📊 Данные WB за "&TEXTJOIN(", ";1;UNIQUE(FILTER({WBP};{WBP}<>"""")))'
           f'&" · считаем ЗАКАЗЫ (выкуп — получение на ПВЗ), выручка — оценка по среднему чеку")')
# — платформы —
